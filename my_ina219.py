...
class INA219:
    ...
    def update_register_pointer(self,address):
        self.bus.write_byte(self.addr,address)
    ...
    def get_config(self):
        self.update_register_pointer(_REG_CONFIG)
        value = self.bus.read_i2c_block_data(self.addr,_REG_CONFIG, 2)
        for byte in value:
            # Convert integer to binary string, remove '0b' prefix, and pad to 8 bits
            binary_representation = format(byte, '08b')
            print(f"Byte in binary: {binary_representation}")

    def set_in_power_down_mode(self):
        power_down_mask = 0xFFF8
        self.update_register_pointer(_REG_CONFIG)
        value = self.bus.read_i2c_block_data(self.addr,_REG_CONFIG, 2)
        combined_value = (value[0] << 8) | value[1]
        print("Before:")
        # Convert integer to binary string, remove '0b' prefix, and pad to 8 bits
        binary_representation = format(combined_value, '016b')
        print(f"Byte in binary: {binary_representation}")                
        self.config = combined_value & power_down_mask
        self.write(_REG_CONFIG,self.config)
        print("After:")
        # Convert integer to binary string, remove '0b' prefix, and pad to 8 bits
        binary_representation = format(self.config, '016b')
        print(f"Byte in binary: {binary_representation}")        
